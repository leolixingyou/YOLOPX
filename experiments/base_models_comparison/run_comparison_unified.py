...
# 创建修改版的run_conflict_comparison_experiment.py
def create_modified_experiment_script():
    """创建一个修改版的实验脚本，使用统一数据集"""
...
def main():
    ...
    # 加载配置
    data_config = edict(yaml.safe_load(open('experiments/base_models_comparison/configs/bdd100k_experiment.yaml')))
    train_config = edict(yaml.safe_load(open('cfgs/train_v2.yaml')))
    ...
    # 保存结果
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_file = f'experiments/base_models_comparison/results/comparison_results_{timestamp}.json'
...
'''
    ...
    script_path = '/workspace/YOLOPX/experiments/base_models_comparison/run_unified_experiment.py'
...
""")
    ...
    # 运行实验
    result = subprocess.run(cmd, cwd='/workspace/YOLOPX/v2')
    
    if result.returncode == 0:
        print("\n✅ 实验成功完成！")
        print("请查看 wandb 项目 'yolop-conflict-analysis' 获取详细结果")
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', action='store_true', help='调试模式（少量数据）')
    args = parser.parse_args()
