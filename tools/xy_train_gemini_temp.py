...
def run_experiment(conflict_method, shared_resources):
    ...
    metrics_for_plotting = {
        'train_total_loss_avg': [], # Renamed to match the average name
        'task_conflict_intensity_avg': [], # Renamed to match the average name
        'epoch_num': []
    }
    ...
    for epoch in range(begin_epoch + 1, begin_epoch + learn_epoch + 1):
        ...
        # Collect metrics for plotting at the end of each epoch
        ...
        # Log epoch-averaged metrics to wandb
        if wandb_run is not None:
            log_epoch_metrics = {
                'epoch': epoch,
                'train/total_loss': epoch_metrics.get('train_total_loss_avg', float('nan')),
                'train/det_loss': epoch_metrics.get('train_det_loss_avg', float('nan')),
                'train/da_seg_loss': epoch_metrics.get('train_da_seg_loss_avg', float('nan')),
                'train/ll_seg_loss': epoch_metrics.get('train_ll_seg_loss_avg', float('nan')),
                'train/task_conflict_intensity': epoch_metrics.get('task_conflict_intensity_avg', float('nan'))
            }
            wandb_run.log(log_epoch_metrics)
            logger.info(f"Epoch {epoch} Wandb log: {log_epoch_metrics}")
        ...
        if epoch >= cfg.TRAIN.END_EPOCH - 1: # Perform validation only at the end
            ...
            if wandb_run is not None:
                wandb_run.log({
                    ...
                    'nms_time': times[1],
                    'epoch': epoch # Log epoch again for validation metrics if not already grouped
                })
    ...
    try:
        ...
        if final_plots:
            ...
            for plot_name, fig in final_plots.items():
                ...
                if wandb_run is not None:
                    # Log final plots as artifacts or images
                    wandb_run.log({f"final_analysis_plots/{plot_name}": wandb.Image(fig)})
...
def train_fixed(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup, logger, 
    ...
    # Get the console handler to temporarily remove it
    ...
    if console_handler:
        logger.removeHandler(console_handler) # Temporarily remove console handler
    ...
    for i, (input, target, paths, shapes) in enumerate(train_pbar):
        ...
        # Update tqdm postfix instead of logging to console
        ...
        if wandb_run is not None:
            ...
            if conflict_solver is not None:
                ...
                log_dict.update({f"batch/{k}": v for k, v in solver_info.items()}) # Add prefix for clarity
            
            wandb_run.log(log_dict)
        
        start = time.time()
    
    # Re-add the console handler after the tqdm loop
    if console_handler:
        logger.addHandler(console_handler)

    # Log end of epoch summary to logger (which now includes console)
...
def main_optimized():
    ...
    metrics_to_plot = {
        ...
        # Add other aggregated metrics from epoch_metrics_accumulator if needed:
        # 'train_det_loss_avg': 'Detection Training Loss',
        # 'train_da_seg_loss_avg': 'Driving Area Segmentation Training Loss',
        # 'train_ll_seg_loss_avg': 'Lane Line Segmentation Training Loss',
    }
    ...
    for metric_key, metric_title in metrics_to_plot.items():
        ...
        for method_name, metrics in all_experiment_metrics.items():
            ...
            values = metrics.get(metric_key) # Get the averaged metric from the collected metrics
        ...
        if has_valid_data:
            ...
            if WANDB_AVAILABLE: # Removed wandb.run is not None check here, assume it's the main process for final plots
                try:
                    # Log the plot to the main WandB run (if it's the last one) or a separate summary run if desired
                    # For individual runs, the run_experiment function logs its own final plots.
                    # This section is for a consolidated comparison plot across all methods.
                    # If this main_optimized is run within its own wandb.run, it would log here.
                    # Otherwise, you might need to re-initialize a 'summary' wandb run here.
                    # For simplicity, assuming this is run in a context where the last wandb_run (or a new one) is active.
                    
                    # To ensure this plot is logged, you'd need a global wandb_run or re-initialize a summary run.
                    # For now, let's assume if WANDB_AVAILABLE is True, there's a way to log globally or to the last run.
                    # A more robust solution might involve creating a dedicated wandb run for these comparison plots.
                    # For simplicity, if a run is currently active, it will log.
                    if wandb.run: # Check if any wandb run is currently active
                        ...
                        print("No active WandB run to log comparative plots to.")
