...
def detect(cfg,opt):
    ...
    for i, (path, img, img_det, vid_cap,shapes) in tqdm(enumerate(dataset),total = len(dataset)):
        ...
        if fps_counter % 30 == 0:
            ...
            print(f"[PyTorch] Frame {fps_counter}: "
                  f"FPS={avg_fps:.2f}, "
    ...
    print(f"Average total time per frame: {(avg_inference_ms + avg_postprocess_ms):.2f}ms")
    print('Results saved to %s' % Path(opt.save_dir))

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='/workspace/onnx_converter/epoch-195.pth', help='model.pth path(s)')
