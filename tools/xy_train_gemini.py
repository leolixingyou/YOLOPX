...
from lib.core.general import non_max_suppression, check_img_size, box_iou, ap_per_class
from lib.utils.utils import time_synchronized

# Import the new log manager
from log_manager import WandBLogger, LocalFileLogger, ConsoleLogger, _coco80_to_coco91_class, xywh2xyxy, scale_coords
...
def validate(epoch, config, val_loader, val_dataset, model, criterion, output_dir, console_logger_instance, wandb_logger_instance):
    """Validation function - enhanced metric logging and WandB image logging"""
    logger = console_logger_instance.get_logger() # Get logger from the ConsoleLogger instance
    ...
    # Get class names for detection for WandB
    names = {k: v for k, v in enumerate(model.names if hasattr(model, 'names') else model.module.names)}
    
    # For JSON output if config.TEST.SAVE_JSON is True
    jdict = []
    is_coco = False # This might need to be determined dynamically if using COCO dataset, currently hardcoded
    coco91class = _coco80_to_coco91_class() # Use imported _coco80_to_coco91_class
    seen = 0
    training = False # Validation mode, so training is False
    ...
    with torch.no_grad():
        for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader), desc='Validation'):
            ...
            for si, pred in enumerate(output):
                ...
                # W&B logging for detections
                wandb_logger_instance.cache_valid_detection_image(img[si], pred.tolist(), names, epoch, path.name)

                # W&B logging for segmentation
                if da_seg_out is not None and da_predict is not None:
                    da_pred_mask_np = da_predict[si].cpu().numpy()
                    wandb_logger_instance.cache_valid_seg_image(img[si], da_pred_mask_np, 'da_seg', epoch, path.name, padding_info=(img.shape[2:], (pad_h, pad_w)))

                if ll_seg_out is not None and ll_predict is not None:
                    ll_pred_mask_np = ll_predict[si].cpu().numpy()
                    wandb_logger_instance.cache_valid_seg_image(img[si], ll_pred_mask_np, 'll_seg', epoch, path.name, padding_info=(img.shape[2:], (pad_h, pad_w)))


                # Append to text file
                if config.TEST.SAVE_TXT:
                    # Note: xywh2xyxy and scale_coords are used implicitly here via predn
                    # These helper functions are now in log_manager.py
                    gn = torch.tensor(shapes[si][0])[[1, 0, 1, 0]]
                    scaled_pred_xywh = (xywh2xyxy(pred[:, :4]) / gn).view(-1).tolist() # Original pred is xyxy, convert to xywh then normalize
                    # This line needs pred to be xywh already OR convert pred to xywh first if it's xyxy
                    # Given `non_max_suppression` typically outputs xyxy, need to convert to xywh before gn
                    # Let's use `predn` which is already scaled to original image space
                    xywh_norm = (xywh2xyxy(predn[:, :4]) / gn).view(-1).tolist() # predn is scaled xyxy, convert to xywh and normalize
                    # The original `function.py` uses predn[:,:4] for box_iou but pred for `box_data` (raw output)
                    # and predn.tolist() for save_txt. Let's stick to `predn` for save_txt.
                    line_data = (predn[:, 5].cpu().tolist()[0], *xywh_norm[:4], predn[:, 4].cpu().tolist()[0]) # cls, x, y, w, h, conf
                    LocalFileLogger(output_dir).save_detection_txt(
                        predn.tolist(), shapes[si], path.stem # pass predn directly, save_detection_txt will extract xyxy and convert
                    )

                # Append to pycocotools JSON dictionary
                if config.TEST.SAVE_JSON:
                    coco_pred_entry = {'image_id': int(path.stem) if path.stem.isnumeric() else path.stem,
                                       'category_id': coco91class[int(predn[0, 5])] if is_coco else int(predn[0, 5]), # Assuming one pred for simplicity, iterate for all
                                       'bbox': [round(x, 3) for x in xywh2xyxy(predn[:,:4])[0].tolist()], # convert scaled xyxy to xywh for JSON
                                       'score': round(predn[0, 4].item(), 5)}
                    jdict.append(coco_pred_entry)
    ...
    # Log sample images to WandB after all stats are computed
    wandb_logger_instance.log_validation_images(epoch)

    # Plots (confusion matrix) and JSON saving
    local_file_logger = LocalFileLogger(output_dir) # Re-instantiate for correct path, or pass from run_experiment
    if config.TEST.PLOTS:
        local_file_logger.save_confusion_matrix_plot(confusion_matrix, names)
    
    if config.TEST.SAVE_JSON and len(jdict) > 0:
        local_file_logger.save_detection_json(jdict)
...
def train_fixed(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup, console_logger_instance, 
    ...
    console_logger_instance.remove_console_handler()
    ...
    for i, (input, target, paths, shapes) in enumerate(train_pbar):
        ...
        with amp.autocast(enabled=device.type != 'cpu'):
            ...
            if total_loss_raw is None or head_losses_raw is None:
            ...
            if conflict_solver is not None:
                ...
                head_losses_raw, shared_params, scaler, num_iter
                )
        ...
        if i % cfg.PRINT_FREQ == 0:
            postfix_str = (f'Loss {epoch_metrics_accumulator["train_total_loss"].val:.5f} ({epoch_metrics_accumulator["train_total_loss"].avg:.5f})'
                           f'| TCI={epoch_metrics_accumulator["task_conflict_intensity"].val:.3f}' if epoch_metrics_accumulator["task_conflict_intensity"].count > 0 else "")
            
            train_pbar.set_postfix_str(postfix_str)

        if wandb_logger_instance:
            log_dict = {
                'batch/train_total_loss': final_loss_for_backward.item(),
            ...
            if len(head_losses_raw) >= 3:
                log_dict.update({
                    'batch/train_det_loss': head_losses_raw[0].item() if head_losses_raw[0] is not None else float('nan'),
                    'batch/train_da_seg_loss': head_losses_raw[1].item() if head_losses_raw[1] is not None else float('nan'),
                    'batch/train_ll_seg_loss': head_losses_raw[2].item() if head_losses_raw[2] is not None else float('nan'),
                })
            ...
            if conflict_solver is not None:
                ...
                log_dict.update({f"batch/{k}": v for k, v in solver_info.items()})
            
            wandb_logger_instance.log_batch_metrics(log_dict)
        
        start = time.time()
    
    console_logger_instance.add_console_handler()

    logger.info(f'Epoch {epoch} finished. Avg Loss: {epoch_metrics_accumulator["train_total_loss"].avg:.5f}, '
                f'Avg TCI: {epoch_metrics_accumulator["task_conflict_intensity"].avg:.3f}')
...
def main_optimized():
    ...
    for method in methods:
        ...
        # Initialize loggers for this experiment run
        time_str = time.strftime('%Y%m%d-%H%M%S')
        run_id = f"run-{time_str}-{hash(time.time()) % 10000:04d}"
        method_suffix_log = f"_{method_name}"
        log_dir_path = Path(cfg.LOG_DIR) / cfg.DATASET.DATASET / f'{run_id}{method_suffix_log}'
        log_dir_path.mkdir(parents=True, exist_ok=True)

        console_logger_instance = ConsoleLogger(str(log_dir_path), f'{run_id}{method_suffix_log}')
        current_experiment_logger = console_logger_instance.get_logger()
        
        wandb_logger_instance = WandBLogger(cfg, f"{run_id}{method_suffix_log}", project_name=f"multitask-training-{cfg.DATASET.DATASET}_refactor", reinit=True)
        wandb_logger_instance.set_logger(current_experiment_logger)
        
        local_file_logger_instance = LocalFileLogger(str(log_dir_path))
    ...
    for metric_key, metric_title in metrics_to_plot.items():
        current_experiment_logger.info(f"Generating comparative plot for: {metric_title}...") # Use the logger
        fig_comp, ax_comp = plt.subplots(figsize=(12, 7)) # Create figure and axes
        
        has_valid_data = False
        for method_name, metrics in all_experiment_metrics.items():
            
            values = metrics.get(metric_key)
            epochs = metrics['epoch_num'] 

            if values is None or not isinstance(values, list) or not any(~np.isnan(v) for v in values):
                current_experiment_logger.warning(f"No valid data for {metric_title} for method {method_name}. Skipping plot for this method on this metric.")
                continue
            ...
            if np.any(valid_indices):
                ax_comp.plot(np.array(epochs)[valid_indices], np.array(values)[valid_indices], label=method_name, linewidth=1.5)
            else:
                current_experiment_logger.warning(f"All values are NaN for {metric_title} for method {method_name}. Skipping plot for this method on this metric.")

        if has_valid_data:
            ax_comp.set_title(f'{metric_title} Comparison Across Methods', fontsize=16, fontweight='bold')
            ax_comp.set_xlabel('Epoch', fontsize=12)
            ax_comp.set_ylabel(metric_title, fontsize=12)
            ax_comp.grid(True, linestyle='--', alpha=0.6)
            ax_comp.legend(loc='upper right', fontsize=10)
            fig_comp.tight_layout()
            
            local_file_logger_instance.save_comparison_plot(fig_comp, f"MTL_comparison_{metric_key}")
            
            if wandb_logger_instance.wandb_run: # Check if wandb is available and run is active
                wandb_logger_instance.wandb_run.log({f"comparative_plots/{metric_key}": wandb.Image(fig_comp), "epoch": cfg.TRAIN.END_EPOCH})
                current_experiment_logger.info(f"Comparative plot also logged to WandB: {metric_key}")
            
            plt.close(fig_comp)
        else:
            current_experiment_logger.warning(f"No valid data to generate comparative plot for {metric_title}.")
...
def run_experiment_isolated(conflict_method, shared_resources, console_logger_instance, wandb_logger_instance, local_file_logger_instance):
    ...
    for epoch in range(begin_epoch + 1, begin_epoch + learn_epoch + 1):
        epoch_metrics = train_fixed(cfg, train_loader, model_copy, criterion_proto, optimizer_copy, scaler_copy,
             epoch, num_batch, num_warmup, console_logger_instance, device, wandb_logger_instance, 
             conflict_detector_exp, conflict_solver_exp, max_epch=begin_epoch + learn_epoch + 1)
        ...
        wandb_logger_instance.log_epoch_metrics(epoch_metrics, epoch)
        logger.info(f"Epoch {epoch} Wandb log: {epoch_metrics}")

        if epoch >= cfg.TRAIN.END_EPOCH - 1: # Perform validation only at the end
            da_results, ll_results, detect_results, total_loss, _, times = validate(
                epoch, cfg, valid_loader, valid_dataset, model_copy, criterion_proto,
                str(local_file_logger_instance.log_dir), console_logger_instance, wandb_logger_instance
            )
            ...
            wandb_logger_instance.log_epoch_metrics(val_log_dict, epoch)
    ...
    try:
        ...
        if final_plots:
            plots_dir = local_file_logger_instance.log_dir / 'plots'
            plots_dir.mkdir(exist_ok=True)
            ...
            for plot_name, fig in final_plots.items():
                ...
                wandb_logger_instance.log_final_plots(plot_name, fig)
                
                plt.close(fig)
            
            wandb_logger_instance.log_training_summary(conflict_detector_exp.get_training_summary())
    ...
    final_model_file = local_file_logger_instance.log_dir / 'final_state.pth'
    model_state = model_copy.module.state_dict() if is_parallel(model_copy) else model_copy.state_dict()
    torch.save(model_state, final_model_file)
    logger.info(f"Final model saved to: {final_model_file}")
    
    # wandb_logger_instance.finish_run() # This is handled in main_optimized for each experiment run
