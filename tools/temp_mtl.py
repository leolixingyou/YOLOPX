...
class TaskHeadConflictDetector:
    """多任务头冲突检测器 - 核心冲突检测模块"""
    ...
    def __init__(self, task_names: List[str], device: str = 'cuda'):
        ...
        # 冲突检测历史
        self.conflict_matrix_history = []
        self.backbone_influence_history = defaultdict(list)
        self.task_head_gradients_history = defaultdict(list)
        self.learning_interference_history = defaultdict(list)
        self.task_convergence_rates = defaultdict(list)
        
    def detect_task_head_conflicts(self, 
                                 losses: Dict[str, torch.Tensor], 
                                 model: nn.Module,
                                 step: int) -> Dict[str, float]:
        """检测任务头之间的冲突"""
        
        # 1. 分离backbone和task heads参数
        backbone_params = []
        task_head_params = {}
        
        for name, module in model.named_modules():
            if 'shared' in name or 'backbone' in name:
                backbone_params.extend(list(module.parameters()))
            elif any(task in name for task in self.task_names):
                task_name = next(task for task in self.task_names if task in name)
                if task_name not in task_head_params:
                    task_head_params[task_name] = []
                task_head_params[task_name].extend(list(module.parameters()))
        
        # 2. 计算每个任务对backbone的梯度影响
        task_to_backbone_grads = {}
        backbone_grad_magnitudes = {}
        
        for task, loss in losses.items():
            if len(backbone_params) > 0:
                try:
                    task_grads = torch.autograd.grad(
                        loss, backbone_params, 
                        retain_graph=True, 
                        create_graph=False,
                        allow_unused=True
                    )
                    
                    valid_grads = [g.flatten() for g in task_grads if g is not None]
                    if valid_grads:
                        task_backbone_grad = torch.cat(valid_grads)
                        task_to_backbone_grads[task] = task_backbone_grad
                        backbone_grad_magnitudes[task] = torch.norm(task_backbone_grad).item()
                    else:
                        total_params = sum(p.numel() for p in backbone_params if p.requires_grad)
                        task_to_backbone_grads[task] = torch.zeros(total_params, device=self.device)
                        backbone_grad_magnitudes[task] = 0.0
                except Exception as e:
                    print(f"Warning: Gradient computation failed for {task}: {e}")
                    total_params = sum(p.numel() for p in backbone_params if p.requires_grad)
                    task_to_backbone_grads[task] = torch.zeros(total_params, device=self.device)
                    backbone_grad_magnitudes[task] = 0.0
        
        # 3. 计算任务头自身的梯度大小
        task_head_grad_magnitudes = {}
        for task, loss in losses.items():
            if task in task_head_params and len(task_head_params[task]) > 0:
                try:
                    head_grads = torch.autograd.grad(
                        loss, task_head_params[task],
                        retain_graph=True,
                        allow_unused=True
                    )
                    valid_head_grads = [g.flatten() for g in head_grads if g is not None]
                    if valid_head_grads:
                        head_grad_norm = torch.norm(torch.cat(valid_head_grads)).item()
                    else:
                        head_grad_norm = 0.0
                    task_head_grad_magnitudes[task] = head_grad_norm
                except Exception as e:
                    print(f"Warning: Head gradient computation failed for {task}: {e}")
                    task_head_grad_magnitudes[task] = 0.0
        ...
        for i, task_i in enumerate(self.task_names):
            for j, task_j in enumerate(self.task_names):
                if i != j and task_i in task_to_backbone_grads and task_j in task_to_backbone_grads:
                    grad_i = task_to_backbone_grads[task_i]
                    grad_j = task_to_backbone_grads[task_j]
                    
                    norm_i = torch.norm(grad_i)
                    norm_j = torch.norm(grad_j)
                    
                    if norm_i > 1e-8 and norm_j > 1e-8:
                        cosine_sim = torch.dot(grad_i, grad_j) / (norm_i * norm_j)
                        conflict_matrix[i, j] = cosine_sim.item()
                        
                        # 学习干扰强度 = |负相关| * 梯度大小的几何平均
                        if cosine_sim < 0:
                            interference = abs(cosine_sim.item()) * np.sqrt(norm_i.item() * norm_j.item())
                            learning_interference[f"{task_i}_{task_j}"] = interference
                    else:
                        conflict_matrix[i, j] = 0.0
        
        # 5. 保存历史数据
        self.conflict_matrix_history.append(conflict_matrix)
        for task in self.task_names:
            if task in backbone_grad_magnitudes:
                self.backbone_influence_history[task].append(backbone_grad_magnitudes[task])
            if task in task_head_grad_magnitudes:
                self.task_head_gradients_history[task].append(task_head_grad_magnitudes[task])
        
        for pair, interference in learning_interference.items():
            self.learning_interference_history[pair].append(interference)
        
        # 6. 构建返回指标
        metrics = {
            # 整体冲突指标
            'overall_conflict_intensity': float(np.mean(np.abs(conflict_matrix[conflict_matrix < 0])) if np.any(conflict_matrix < 0) else 0.0),
            'positive_alignment_ratio': float(np.mean(conflict_matrix > 0.1)),
            'severe_conflict_pairs': int(np.sum(conflict_matrix < -0.5)),
            
            # 任务级别指标
            **{f'backbone_influence_{task}': backbone_grad_magnitudes.get(task, 0.0) 
               for task in self.task_names},
            **{f'head_learning_rate_{task}': task_head_grad_magnitudes.get(task, 0.0) 
               for task in self.task_names},
            
            # 任务对冲突指标
            **{f'conflict_{task_i}_{task_j}': conflict_matrix[i, j]
               for i, task_i in enumerate(self.task_names)
               for j, task_j in enumerate(self.task_names) if i != j},
            
            # 学习干扰指标
            **{f'interference_{pair}': interference 
               for pair, interference in learning_interference.items()},
        }
    ...
    def get_conflict_summary(self) -> Dict[str, float]:
        """获取冲突检测摘要"""
        if len(self.conflict_matrix_history) == 0:
            return {}
        
        latest_matrix = self.conflict_matrix_history[-1]
        
        summary = {
            'total_conflict_pairs': int(np.sum(latest_matrix < -0.1)),
            'severe_conflict_pairs': int(np.sum(latest_matrix < -0.5)),
            'cooperative_pairs': int(np.sum(latest_matrix > 0.3)),
            'average_conflict_intensity': float(np.mean(np.abs(latest_matrix[latest_matrix < 0])) if np.any(latest_matrix < 0) else 0.0),
            'max_conflict': float(np.min(latest_matrix)),
            'max_cooperation': float(np.max(latest_matrix)),
        }
        
        # 找出最冲突的任务对
        if latest_matrix.size > 0:
            min_idx = np.unravel_index(np.argmin(latest_matrix), latest_matrix.shape)
            max_idx = np.unravel_index(np.argmax(latest_matrix), latest_matrix.shape)
            
            summary['most_conflicting_pair'] = f"{self.task_names[min_idx[0]]}_{self.task_names[min_idx[1]]}"
            summary['most_cooperative_pair'] = f"{self.task_names[max_idx[0]]}_{self.task_names[max_idx[1]]}"
...
class MTLConflictResolver(ABC):
    ...
    def __init__(self, task_names: List[str], device: str = 'cuda'):
        ...
        # 添加任务头冲突检测器
        self.conflict_detector = TaskHeadConflictDetector(task_names, device)
    ...
    def detect_and_update_weights(self, 
                                losses: Dict[str, torch.Tensor], 
                                model: nn.Module,
                                epoch: int, 
                                step: int) -> Tuple[Dict[str, float], Dict[str, float]]:
        """更新权重并检测任务头冲突"""
        # 原方法的权重更新
        task_weights = self.update_weights(losses, epoch, step)
        
        # 任务头冲突检测
        conflict_metrics = self.conflict_detector.detect_task_head_conflicts(losses, model, step)
        
        return task_weights, conflict_metrics
    ...
    def get_enhanced_metrics(self) -> Dict[str, float]:
        ...
        # 合并指标
        enhanced_metrics = {**base_metrics, **conflict_summary}
