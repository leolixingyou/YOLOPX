...
class TrapezoidFusionReceiver:
    ...
    def calculate_change(self, current, history):
        """计算变化幅度 - 更稳定的版本"""
    ...
    def weighted_average_trapezoid(self, trap1, trap2):
        ...
        try:
            ...
            # 如果历史数据不足，使用固定权重
            if len(self.img_history) < 5 or len(self.lidar_history) < 5:
            ...
            rospy.loginfo(f"Ultra-Stable Weights - IMG: {smooth_w1:.3f}, LiDAR: {smooth_w2:.3f}")
            return final_fused
