...
class AutoDriveDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, is_train, transform=None):
        ...
        self.db = []
        self.class_names = cfg.DATASET.NAMES
    ...
    def __getitem__(self, index):
        ...
        return img, labels, seg_label, lane_label, data['image'], self.class_names

    @staticmethod
    def collate_fn(batch):
        img, label, seg_label, lane_label, paths, _ = zip(*batch)
        label_new = []
...
class BddDataset(AutoDriveDataset):
    def __init__(self, cfg, is_train, transform=None):
        ...
        self.id_dict = {name: i for i, name in enumerate(self.class_names)}
